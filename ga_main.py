...
'''
from ga_4layers_new_metric import *
...
'''
    ...
    if (elem[layer_index]+0.5)<=myConst.max_DLC_thickness:
        priority_value=elem[0]
        elem=elem[:layer_index]+(elem[layer_index]+0.5, ) + elem[(layer_index+1):]
...
#selection of parents for crossover and mutation
        ...
        parent_one_=probability.rvs()
        parent_two_=probability.rvs()
...
def output_TR(fname, check=True):
    ...
    if check:
        TR(rev, min_wl=200, max_wl=2500, show_solar=True, legend=False)
        plot.plt.grid('on')
        show()
    os.chdir("..")
