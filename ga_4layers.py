...
myConst=constants(lower_limit=390, upper_limit=700, T_factor=.55, R_factor=.45, max_DLC_thickness=140.0)


## FOR correct_TR()
glass_ext = np.loadtxt("glass_extinct_assume_n1.5.txt")
##wl_A, A = glass_ext[2:-25,0], glass_ext[2:-25,1]    #210nm to 2475nm
wl_A, A = glass_ext[6:-25,0], glass_ext[6:-25,1]    #210nm to 2475nm for AlN
Tg = 0.96
Rg = 0.04
...
'''
    ...
    LAg_thickness=random.uniform(3.0, 20.0)
    LAg=float("{0:.1f}".format(LAg_thickness))

        
    LDLC_thickness=random.uniform(20.0, myConst.max_DLC_thickness)
    LDLC=float("{0:.1f}".format(LDLC_thickness))
    ...
    # from 1. 
    child_one = (None,) + parent_one[1:3] + (LAg,) + parent_two[4:5]
    child_two = (None,) + parent_two[1:3] + parent_one[3:4] + (LDLC,)
