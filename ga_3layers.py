...
myConst=constants(lower_limit=390, upper_limit=700, T_factor=.55, R_factor=.45, max_DLC_thickness=200.0)
...
'''
    ...
    child_one=parent_one[0:2]+(ML_thickness, BL_thickness)
    
    child_two=parent_two[0:2]+(parent_one[2], BL_thickness)

    child_three=child_one[:1]+child_one[3:]+child_one[2:3]+child_one[1:2]
    
    child_four=child_two[:1]+child_two[3:]+child_two[2:3]+child_two[1:2]
